...
#define mouse_x     (volatile int*) MOUSE_X_BASE
#define mouse_y     (volatile int*)	MOUSE_Y_BASE
#define mouse_click (volatile alt_u8*)	MOUSE_CLICK_BASE
...
volatile unsigned int * rand1=0xd0;
volatile unsigned int * rand2=0xc0;
 ...
 //               printf("[SIE1 MSG]:SIE1 msg reg is %x\n", usb_ctl_val);
        ...
        else
        {
            if (data_size != 8)
            {
                ...
                //printf("the button_value at 0x051c is: 0x%02x\n", button_value);
                /*if ((button_value & 0xff) == 0x01)
                {
                    printf("left button\n");
                    pbutton = 1;
                }
                else if ((button_value & 0xff) == 0x02)
                {
                    printf("right button\n");
                    pbutton = 2;
                }
                ...
                //printf("the button_value at 0x051d is: 0x%02x\n", button_value);
                /*if ((signed char)((button_value) & 0xff) != 0)
                {

                    if ((signed char)((button_value) & 0xff) == dx)
                    {
                        dx = 0;
                    }
                    else
...
//                printf("the button_value for dy (at 0x051e) is: 0x%02x\n", button_value);
                ...
                /*if (button_value & 0xff)
                {
                    ...
                    if ((signed char)(button_value & 0xff) == dy)
                    {
                        dy = 0;
                    }
                    else
                ...
                /*IOWR(CY7C67200_BASE, HPI_ADDR, 0x0520); //the start address
                read = (signed short)IORD(CY7C67200_BASE, HPI_DATA);
                printf("the read from 0x0520 is: 0x%04x\n", read);

                IOWR(CY7C67200_BASE, HPI_ADDR, 0x0522); //the start address
                read = (signed short)IORD(CY7C67200_BASE, HPI_DATA);
                printf("the read from 0x0522 is: 0x%04x\n", read);

                IOWR(CY7C67200_BASE, HPI_ADDR, 0x0524); //the start address
                read = (signed short)IORD(CY7C67200_BASE, HPI_DATA);
                printf("the read from 0x0524 is: 0x%04x\n", read);

                IOWR(CY7C67200_BASE, HPI_ADDR, 0x0526); //the start address
                read = (signed short)IORD(CY7C67200_BASE, HPI_DATA);
                printf("the read from 0x0526 is: 0x%04x\n", read);

                IOWR(CY7C67200_BASE, HPI_ADDR, 0x0528); //the start address
                read = (signed short)IORD(CY7C67200_BASE, HPI_DATA);
                printf("the read from 0x0528 is: 0x%04x\n", read);

                IOWR(CY7C67200_BASE, HPI_ADDR, 0x052a); //the start address
                read = (signed short)IORD(CY7C67200_BASE, HPI_DATA);
                printf("the read from 0x052a is: 0x%04x\n", read);

                IOWR(CY7C67200_BASE, HPI_ADDR, 0x052c); //the start address
                read = (signed short)IORD(CY7C67200_BASE, HPI_DATA);
                printf("the read from 0x052c is: 0x%04x\n", read);

                IOWR(CY7C67200_BASE, HPI_ADDR, 0x051a); //the start address
                read = (signed short)IORD(CY7C67200_BASE, HPI_DATA);
                printf("the read from 0x051a is: 0x%04x\n", read);

                IOWR(CY7C67200_BASE, HPI_ADDR, 0x0510); //the start address
                read = (signed short)IORD(CY7C67200_BASE, HPI_DATA);
                printf("the read from 0x0510 is: 0x%04x\n", read);

                IOWR(CY7C67200_BASE, HPI_ADDR, 0x0508); //the start address
                read = (signed short)IORD(CY7C67200_BASE, HPI_DATA);
                printf("the read from 0x0508 is: 0x%04x\n", read);

                IOWR(CY7C67200_BASE, HPI_ADDR, 0x0506); //the start address
                read = (signed short)IORD(CY7C67200_BASE, HPI_DATA);
                printf("the read from 0x0506 is: 0x%04x\n", read);

                IOWR(CY7C67200_BASE, HPI_ADDR, 0x0504); //the start address
                read = (signed short)IORD(CY7C67200_BASE, HPI_DATA);
                printf("the read from 0x0504 is: 0x%04x\n", read);

                IOWR(CY7C67200_BASE, HPI_ADDR, 0x0502); //the start address
                read = (signed short)IORD(CY7C67200_BASE, HPI_DATA);
                printf("the read from 0x0502 is: 0x%04x\n", read);
